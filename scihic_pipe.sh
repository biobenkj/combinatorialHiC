...
outdir=$6 #outdir
bwt2idx=$7 #add a bowtie2 index path
procs=$8 #the number of processors to use
genomesize=$9 #genome or chrom.sizes file
#MAIN
...
python2 inline_splitter.py /tmp/$fq_r1.clipped /tmp/$fq_r2.clipped $barcodes /tmp/$fq_r1.split /tmp/$fq_r2.split 2> $outdir/splitting_stats.html
#Run analyze_scDHC_V2design.py, which searches for the adaptor and clips it out
python2 analyze_scDHC_V2design.py $bc1 /tmp/$fq_r1.split /tmp/$fq_r2.split /tmp/$fq_r1.bc_clipped /tmp/$fq_r2.bc_clipped > $outdir/$bc_assoc
#Align reads to combo-reference using bowtie2
bowtie2 -x $bwt2idx -p $procs -U /tmp/$fq_r1.bc_clipped 2> $outdir/$fq_r1.mapping_stats | samtools view -bS - > /tmp/$fq_r1.bam&
bowtie2 -x $bwt2idx -p $procs -U /tmp/$fq_r2.bc_clipped 2> $outdir/$fq_r2.mapping_stats | samtools view -bS - > /tmp/$fq_r2.bam&
wait
...
python2 catbed2bedpe.py $bc_assoc.concat.sorted.bed $bc_assoc $bc_assoc.unmatched.bedpe $bc_assoc.matching.html > $bc_assoc.bedpe.mapq0
...
python2 dedupe_scDHC.py $bc_assoc.bedpe.mapq0.sorted > $bc_assoc.bedpe.mapq0.deduped
#Check some QC stats
...
python2 filter_bedpe.py $genomesize $bc_assoc.bedpe.mapq0.deduped > $bc_assoc.bedpe.mapq0.deduped.filtered
...
python2 sort_strandedness.py $bc_assoc.bedpe.mapq0 $bc_assoc.deduped.stats.percent_plots > $bc_assoc.deduped.stats.html&
python2 sort_strandedness.py $bc_assoc.bedpe.mapq0.deduped.filtered $bc_assoc.associated.stats.percent_plots > $bc_assoc.associated.stats.html&
wait
#Compute breakdown of species specificity of barcodes
python2 calculate_cell_distro_long.py $bc_assoc.bedpe.mapq0 > $bc_assoc.percentages 2> $bc_assoc.REoccurrences &
python2 calculate_cell_distro_long.py $bc_assoc.bedpe.mapq0.deduped.filtered > $bc_assoc.deduped.percentages 2> $bc_assoc.deduped.REoccurrences&
wait
...
python2 single_cell_length_distros.py $bc_assoc.deduped.percentages $bc_assoc.bedpe.mapq0.deduped.filtered $bc_assoc > $bc_assoc.single_cell_lengths
#Calculate median cis-trans ratios from these distributions
python2 calculate_median_cistrans_ratio.py $bc_assoc.single_cell_lengths > $bc_assoc.cistrans.txt
#Incorporate cistrans ratios in the percentages file
python2 incorporate_cistrans_genotype.py $bc_assoc.cistrans.txt $bc_assoc.deduped.percentages > $bc_assoc.deduped.percentages.filterable
#Generate single-cell matrices
python2 bin_schic.py $genomesize $bc_assoc.deduped.percentages.filterable $bedpe.mapq0.deduped.filtered > foo
rm foo
